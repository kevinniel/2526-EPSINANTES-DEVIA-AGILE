- Equipe 1 (https://github.com/ArthurPoncin/agile-devops-tp)
- Equipe 2 (https://github.com/FatoumataDIABATE/TP_AGILE_DEVOPS_GR2)
- Equipe 3 (https://github.com/orgs/HealthAI-Corpo/projects/3)
- Equipe 4 (https://github.com/KillianGascon/Agilit--GestionEvents)
- Equipe 5 (https://github.com/Sblaaaf/DEVIA-AGILE-GR5)

# 19/05/26 matin
- Equipe 1 : Sprint & US sont en place dans la backlog. Daily faite. Ils font une app "LBC" du style imobilier uniquement. On se connecte, on dépose une annonce et en tant qu'acheteur on peut acheter. Ils voulaient intégrer l'IA pour que l'IA va automatiquement faire de belle descriptions via Mistral. L'authentification est en ligne. Ils sont parti sur next.js en déployant sur vercel.
- Equipe 2 : Question sur le backlog ce matin. Le backlog et les US sont OK avec les Epics. La base du projet est en place. La demo sera faite jeudi pour avoir le temps de faire les premières fonctionnalités.
- Equipe 3 : Ils vont faire un POC d'une IA pour démontrer la faisabilité autour du workout / calories. Ils ont réalisé leur 14 US et défini les sprints. Ils ne sont pas encore trop sûr de la priorité des sprints. Ils ont ensuite mis en place le kanban. Ils ont bien fait les DoD et DoR. Pas de démo ce matin, on prévoit plus pour jeudi aprem pour la première demo.
- Equipe 4 : US & sprints en place. Ils ont commencé à implémenter les US. Pas de démo prévu ce jour parce que pas grand chose de visuel à présenter.
- Equipe 5 : Kanban en place avec les US. Ils sont en train de commencer les dev après la réalisation du product backlog. Démo prévu pour la prochaine fois le temps que les features arrivent.

# 21/05/26 aprem
- Equipe 1 : Ils ont continué d'avancer sur leur projet. Ils ont mis en place les filtres, les favoris. La génération de la description par IA est bugguée sur la démo du jour. La gestion du scrum dans sa globalité semble plutôt bonne, tout est pris en compte visiblement.
- Equipe 2 : sont parti sur une appli de gestion d'événement : un côté admin et un côté visiteur. Ils ont dockerisé l'application. Le kanban semble bien mené, les affectations, tickets et estimation de difficulté des tâches sont bien gérées. Pas de blocage pour le moment.
- Equipe 3 : Je les ai appelé 2 fois en vocal teams + tag dans le chat, et pas de réponse entre 16h et 17h10. J'attends de voir la suite. Ils m'ont contacté à 17h20 et ont pris un peu de retard car un membre est absent. L'organisation est décrite par le PO est compliquée (perte d'efficacité) car 4 au lieu de 5 personnes en place. Ils ont peut etre eu du mal sur la partie estimation de la valeur de la tâche : ça a été estimé trop bas en terme de complexité. Ils pensent essayer de rattraper leur retard demain matin.
- Equipe 4 : Ils ont fait une gestion d'événements persos / groupe / famille / travail + export calendar .ics. Le Kaban est OK dans l'ensemble, mais les tags ne sont pas utilisés pour la partie PP ou autres informations de tri. Ils ont mis en place des labels que je n'avais pas vu pour la partie PP. Les milestones sont bien en place.
- Equipe 5 : démo de leur outil qui permet soit de faire une musique. Ils ont un souci qu'ils ne comprennent pas, qui fait que certains navigateurs et certaines catégories font que les musiques ne se déclenchent pas. Ils ont fait un outil de blind test ooù la musique peut être jouée à l'envers. Ils ont refait des tickets / US pour traiter ces bugs, en les intégrant dans le backlog et le kanban. Musiques via API deezer en random. Le déploiement n'est pas une priorité, mais ça serait cool si ils pouvaient mettre un CI/CD + déploiement. Le suivi kanban n'est pas optimal, il manque pas mal d'infos dans les cards - il faudra les préciser ou y prêter attention.

# 22/05/26 aprem
- Equipe 1 :
- Equipe 2 :
- Equipe 3 :
- Equipe 4 :
- Equipe 5 :
